...
export class ConsumerSqsService {
  ...
  async handleMessage(message: AWS.SQS.Message) {
    ...
    (
      await this.sendSQSMessengeToService(
        this.getEventMessageSQS(JSON.parse(message.Body)),
      )
    ).data;
  }
  ...
  //   ).data;
  // }
  @SqsMessageHandler('PCO-SC-TESTCONCEPTSNSTOPIC.fifo', false)
  async handleMessageFIFO(message: AWS.SQS.Message) {
    // console.log(JSON.parse(message.Body))
    // this.sqsMessage = JSON.parse(message.Body);
    console.log(this.getEventMessageSQS(JSON.parse(message.Body)));
  ...
  // @SqsMessageHandler('MicroServicerReadSQS.fifo', true)
  // async handleMessageFIFOLotes(message: AWS.SQS.Message) {
  //   // console.log(JSON.parse(message.Body))
  //   // this.sqsMessage = JSON.parse(message.Body);
  //   console.log(message);
  ...
  sendSQSMessengeToService(
    eventSQS: EventSendSNS,
  ): Promise<AxiosResponse<EventSendSNS>> {
    // console.log(eventSQS);
    return this.httpService.axiosRef.post(
      'http://localhost:3000/sns/sqsevent',
      eventSQS,
    );
  }
